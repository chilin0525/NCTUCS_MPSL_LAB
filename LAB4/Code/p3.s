...
GPIO_init:  
    ...
    ldr r0,=0b11111111111111110101011111111111  // set pa5 pa6 pa7 = 01 (output mode)
    ldr r1,=GPIOA_MODER
    str r0,[r1]
    ...
    mov r0,#0xF3FFFFFF                          // set pc13 as input mode
    ldr r1,=GPIOC_MODER
    str r0,[r1]

    mov r0,#0b10000000000000                    // set pc13 as 1 (not pull down)
    ldr r1,=GPIOC_IDR                           // pull-down == 0
    str r0,[r1]

    ldr r0,=0x04000000                          // set pc13 as pull-up 
    ldr r1,=GPIOC_PUPDR
    str r0,[r1]
...
//r0:address,r1:data
    ...
    ldr r0,=#DECODE_MODE
    ldr r1,=#0b11111111     // all digit use code B
    BL MAX7219Send

    ldr r0,=#DISPLAY_TEST
    ldr r1,=#0x0            // set as normal operation
    BL MAX7219Send

    ldr r0,=#SCAN_LIMIT     // display all digit
    ldr r1,=#0x7
    BL MAX7219Send

    ldr r0,=#INTENSITY      // set Intensity as A
    ldr r1,=#0xA
    BL MAX7219Send

    ldr r0,=#SHUTDOWN       // set as normal operation
    ldr r1,=#0x1
    BL MAX7219Send

    mov r2,2//r2:counter
    LOOP_SET_ALL_DIGIT_TO_BLANK_EXCEPT_DIGIT1_2:    //set digit 1~7 to blank
        mov r0,r2               // digit 0-7 : value 2~8 
        lsl r0,#8
    ...
    ldr r0,=0b000100000000      //set digit zero to zero (init value)
    ldr r1,=0
    BL MAX7219Send
